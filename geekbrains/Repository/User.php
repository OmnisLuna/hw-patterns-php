...
class User
{
    ...
    {
        $identityMapSearch = $this->identityMap->find("User", $id);
        // смотрим есть ли такой объект в IdentityMap и если нет, то добавляем его
        if ($identityMapSearch !== null) {
            return $identityMapSearch;
        // если объект уже есть в IdentityMap, то возвращаем его без похода в хранилище
        } else if ($identityMapSearch === null) {
